...
def mtTest() -> List[float]:
    ...
    context = [90, 87, 21, 23, 24, 90, 87, 21, 23, 24]
    key_bytes = key.to_bytes(4, 'big')
    salts = [s.to_bytes(4, 'big') for s in range(1, 28125000)]
...
def mp_work(executor, salts, key_bytes, context) -> List[float]:
    ...
    return list(executor.map(getY_partial, salts, chunksize=math.ceil(len(salts)//executor._max_workers)))
...
if __name__ == "__main__":
    t0 = time.time()
    main()
    print(f"Overhead: {time.time()-t0:.2f}")